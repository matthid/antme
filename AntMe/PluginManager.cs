    ...
    internal sealed class PluginManager {
        ...
        public void SaveSettings() {
            ...
            foreach (PluginItem plugin in producerList.Values) {
                try {
                    File.WriteAllBytes(
                        configPath + plugin.Guid + Resources.PluginSettingsFileExtension, plugin.Producer.Settings);
                }
                catch (Exception ex) {
                    exceptions.Add(
                        new Exception(
            ...
            foreach (PluginItem plugin in consumerList.Values) {
                try {
                    File.WriteAllBytes(
                        configPath + plugin.Guid + Resources.PluginSettingsFileExtension, plugin.Consumer.Settings);
                }
                catch (Exception ex) {
                    exceptions.Add(
                        new Exception(
